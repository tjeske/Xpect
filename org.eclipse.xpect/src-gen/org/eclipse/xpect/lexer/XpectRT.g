...
XPECT_SETUP: {(tokenstate & 1) != 0 && matches("XPECT_SETUP")}?=> 'XPECT_SETUP' {tokenstate=2;};
XPECT: {(tokenstate & 1) != 0 && matches("XPECT")}?=> 'XPECT' {tokenstate=4;};
...
END_SETUP: {(tokenstate & 2) != 0 && matches("END_SETUP")}?=> 'END_SETUP' {tokenstate=1;};
...
RULE_INVOCATION: {(tokenstate & 4) != 0}?=> '^'? ('a'..'z'|'A'..'Z'|'_') ('a'..'z'|'A'..'Z'|'_'|'0'..'9')*  {tokenstate=1;};
